...
with open('merged.json') as data_file:    
    ...
    for v in data:
       ...
       for z in v['ratings']:
          rt=rt+int(z)
       rating=rt/ucount
       session.execute("""INSERT INTO recipes.recipe_details (recipe_id,name,contributor_id,submitted,rating,tags,n_steps,minutes,steps,n_ingredients,ingredients,nutrition,category,description)VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",(int(recipe_id),name,int(contributor_id),submitted,float(rating),tags,int(n_steps),int(minutes),steps,int(n_ingredients),ingredients,nutrition,category,description)) 
